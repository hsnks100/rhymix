<?php
    ...
    class member_group extends WidgetHandler {
        ...
        function proc($args) {
            ...
            // print_r($widget_info->member_list);

            foreach($widget_info->member_list as $key => $value) {
                // print_r( unserialize($value->extra_vars) );
                $profile = $oMemberModel->getProfileImage($value->member_srl);
                // print_r($profile);
                $widget_info->member_list[$key]->extra = unserialize($value->extra_vars); // '123';
                $tt = $profile->src;
                $widget_info->member_list[$key]->profile = $tt;
            }
